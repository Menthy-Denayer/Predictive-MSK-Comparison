...
function update( model )
    ...
    -- extract position of contact force, should be zero if no contact, otherwise < 0
    grf_pos_left = calcn_l:contact_point().y
    grf_pos_right = calcn_r:contact_point().y

    left_penalty = 0
    -- if contact detected during swing on left side
    if( math.abs( vel_l ) > vel_threshold and com_y_l > com_threshold ) then 
        -- left_penalty = math.abs( vel_l * grf_left )
        left_penalty = math.abs( grf_pos_left )
        tot_penalty = tot_penalty + left_penalty
    end 

    right_penalty = 0
    -- if contact detected during swing on right side
    if( math.abs( vel_r ) > vel_threshold and com_y_r > com_threshold ) then 
        -- right_penalty = math.abs( vel_r * grf_right )
        right_penalty = math.abs( grf_pos_right )
